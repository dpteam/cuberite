...
decltype(cRoot::g_RunAsService) cRoot::g_RunAsService = false;

/** Indicate the next action of cRoot, whether to stop or restart */
decltype(cRoot::g_NextState) cRoot::g_NextState = cRoot::NextState::Run;

/** Global that registers itself as a last chance exception handler to write a minidump on crash */
cMiniDumpWriter g_MiniDumpWriter;
...
static void NonCtrlHandler(int a_Signal)
{
	...
	switch (a_Signal)
	{
		...
		case SIGTERM:
		{
			...
			cRoot::Stop(cRoot::NextState::Stop);
			return;
		}
#ifdef SIGPIPE
		case SIGPIPE:
...
static int UniversalMain(int argc, char * argv[], bool RunningAsService)
{
	...
	try
	{
		...
		if (!RunningAsService && cRoot::g_RunAsService)
		{
	...
	{
		std::cerr << fmt::sprintf("Error reading command line {} for argument {}", Oops.error(), Oops.argId());
		return EXIT_FAILURE;
	}
